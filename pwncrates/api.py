...
def is_uuidv4(s):
    try:
        ...
        # Ensure it's actually version 4 and not just valid format
...
def api_start_challenge(challenge_id):
    if not is_uuidv4(str(challenge_id)):
    ...
    response = requests.get(f"{INSTANCER_URL}/start/{user}/{challenge_id}", auth=HTTPBasicAuth(INSTANCER_USERNAME, INSTANCER_PASSWORD))
...
def api_stop_challenge(challenge_id):
    ...
    user = b16encode(db.get_user(user_id=current_user.id)['username'].encode()).decode().lower()
    response = requests.get(f"{INSTANCER_URL}/stop/{user}/{challenge_id}", auth=HTTPBasicAuth(INSTANCER_USERNAME, INSTANCER_PASSWORD))
    return response.text
...
def api_status_challenge(challenge_id):
    ...
    user = b16encode(db.get_user(user_id=current_user.id)['username'].encode()).decode().lower()
    response = requests.get(f"{INSTANCER_URL}/status/{user}/{challenge_id}", auth=HTTPBasicAuth(INSTANCER_USERNAME, INSTANCER_PASSWORD))
    return response.text
...
def api_submit_challenge(challenge_id):
    try:
        ...
        if (app.config["pwncrates"]["WEBHOOK_URL"].startswith("https://") or
                app.config["pwncrates"]["WEBHOOK_URL"].startswith("http://")):
            requests.post(app.config["pwncrates"]["WEBHOOK_URL"], json=data)
